...
int read_values(double *sum) {
  ...
  *sum = 0;
  ...
  scanf("%d", &input); // done correctly here
  while(input != 0) {
    values++;
    *sum += input;
    scanf("%d", &input); //scanf puts the desired input into a memory location, by not refrencing the memory location a seg fault was created.
  }
  return values;
}
