...
int has_cycle(node *head) {
	// Your code goes here:
  node *slow, *fast;
  if(head == NULL) return 0; //empty list has no cycles
  slow = head; // set starting points for both pointers
  fast = head->next;
  while(fast != NULL && fast->next != NULL) {
    if(slow == fast) return 1; //cycle found
    slow = slow->next;  // cyce single the pace
    fast = fast->next->next; // Cycle doubly the pace as slow
  }
...
int main(void) {
  test_has_cycle(); // Interesting... Matthew Tabel
  return 0;
}
