...
1.A we need to pass the size of the array since array decays to pointer when passed to function and we cannot use sizeof to get the size of the array.
1.B No, the original array is not changed since we are sorting a copy of the original array; stored on the heap using malloc.
1.C We need to store our result as a copy of the original array since we want to keep the original array unchanged.
*/
